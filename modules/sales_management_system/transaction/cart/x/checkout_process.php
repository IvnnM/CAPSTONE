...
$query = "SELECT c.CartID, p.ProductName, c.Quantity, c.AddedDate, o.RetailPrice, o.MinPromoQty, o.PromoPrice, o.OnhandID, o.OnhandQty
          FROM CartTb c
...
foreach ($cart_items as $item) {
    if ($item['Quantity'] > $item['OnhandQty']) {
        $insufficient_stock_items[] = $item['ProductName'];
    }
}

// If there are items with insufficient stock, show an error message and prevent checkout
if (!empty($insufficient_stock_items)) {
    $_SESSION['alert'] = "The following items do not have enough stock: " . implode(', ', $insufficient_stock_items);
    $_SESSION['alert_type'] = "danger";
    echo "<script>window.location.href = '../../../../views/customer_view.php#Cart';</script>";
