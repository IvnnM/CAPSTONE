...
$query = "SELECT t.TransacID, t.CustName, t.CustNum, t.CustEmail, 
                 l.Province, l.City, t.DeliveryFee, t.TotalPrice, t.TransactionDate, t.ExpectedDeliveryDate 
          FROM TransacTb t
          JOIN LocationTb l ON t.LocationID = l.LocationID
          WHERE t.CustEmail = :cust_email AND t.Status = 'ToShip' 
          ORDER BY t.TransactionDate DESC";
$stmt = $conn->prepare($query);
$stmt->execute(['cust_email' => $cust_email]);
$to_ship_transactions = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    <div class="container-fluid">
        <div class="sticky-top bg-light pb-2">
            <h3>Your Orders to Receive</h3>
            <!-- Breadcrumb Navigation -->
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="../../../../views/customer_view.php#Overview">Home</a></li>
                    <li class="breadcrumb-item"><a href="./order.php"><span class="status-badge status-pending">Pending</a></li>
                    <li class="breadcrumb-item active" aria-current="page"><span class="status-badge status-accepted">Accepted</span></li>
                    <li class="breadcrumb-item"><a href="./completed.php"><span class="status-badge status-completed">Completed</span></a></li>
                </ol>
            </nav>
            <hr>
        ...
        <!-- Table to display to ship transactions -->
        <div class="table-responsive">
            <table id="transactionTable" class="display table table-light table-hover border-secondary pt-2">
                ...
                <tbody>
                    <?php if (empty($to_ship_transactions)): ?>
                        <tr>
                            <td colspan="9">No to ship transactions found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($to_ship_transactions as $transaction): ?>
                            <tr>
                                ...
                                <td data-label="Address"><?= htmlspecialchars($transaction['Province'] . ', ' . $transaction['City']) ?></td>
                                ...
                                <td data-label="Estimated delivery date">
                                    <?php 
