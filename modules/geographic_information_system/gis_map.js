document.addEventListener('DOMContentLoaded', function () {
  ...
  fetch('../includes/get_location_data.php')  // Corrected path
      ...
      .then(data => {
          ...
          // Render total transactions chart for all provinces by default
          renderTotalTransactionsChart();

          // Initialize transactions chart with default province
          if (data.provinces.length > 0) {
              const defaultProvince = data.provinces[0].Province;
              renderTransactionsChart(defaultProvince);
          }
          ...
          provinceDropdown.addEventListener('change', function () {
              ...
              renderTransactionsChart(selectedProvince);
          });
      });
